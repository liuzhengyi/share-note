...
require_once($webRoot."includes.php");
	...
	// 投票
	$sql = "update review set con_amount = con_amount+1 where rev_id = '$rid' limit 1";
	$res = $dbo->exeUpdate($sql);
	$sql = "insert into vote values(NULL, '$uid', '$rid', 'c');";
	$res = $dbo->exeUpdate($sql);
	$sql = "select con_amount as ca from review where rev_id = '$rid' limit 1";
	$res = $dbo->getRow($sql);
	echo $res['ca'];
	$dbo->close();
